...
spec:
  ...
  command: 'bash -c "git pull origin main && python -m attributions.examples.medical_image_call_exp1 --train_annotator {train_annotator} --test_annotator {test_annotator} --nnunet_folders_path $NNUNET_FOLDERS_PATH --num_epochs $NUM_EPOCHS --device $DEVICE --n_jobs $N_JOBS --shapley_samples $SHAPLEY_SAMPLES --num_train_iterations_per_epoch 50 --num_val_iterations_per_epoch 50"'
  scale: 1
...
def main():
    ...
    output_dir = Path("experiments_yamls/exp1_causal_shift_annotator_combinations")
