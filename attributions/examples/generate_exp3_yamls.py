...
type: job
spec:
  name: causal-shift-train{train_annotator}-test{test_annotator}_exp3
  ...
  command: 'bash -c "git pull origin main && python -m attributions.examples.medical_image_call_exp3 --train_annotator {train_annotator} --test_annotator {test_annotator} --nnunet_folders_path $NNUNET_FOLDERS_PATH --num_epochs $NUM_EPOCHS --device $DEVICE --n_jobs $N_JOBS --shapley_samples $SHAPLEY_SAMPLES --num_train_iterations_per_epoch 50 --num_val_iterations_per_epoch 50"'
  scale: 1
...
def main():
    ...
    output_dir = Path("experiments_yamls/exp3_causal_shift_annotator_combinations")
    ...
    for train_annotator in annotators:
        ...
        yaml_content = YAML_TEMPLATE.format(
            train_annotator=train_annotator,
            test_annotator=train_annotator
        )

        # Create filename
        filename = f"train{train_annotator}_test{train_annotator}.yaml"
    ...
    print(f"Generated {len(annotators)} YAML files in {output_dir}")
