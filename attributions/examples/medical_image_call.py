...
def main(args):
    ...
    #train_nnunet_path = "/tmp/tmpxb9veamk/train_data"
    ...
    test_path = discriminator.preprocess_custom_folder(
        input_images_folder=f'{nnunet_folders_path}/nnUNet_raw/{nnunet_dataset}/imagesTs',# # imagesTr for experiment just changing labels as domain shift
        input_segs_folder=f'{nnunet_folders_path}/nnUNet_raw/{nnunet_dataset}/labelsTs_{args.test_annotator}', #labelsTr_2' for experiment just changing labels as domain shift
        folder_name='test'
     )
    #test_path = "/tmp/tmpsvlsj9d_/test"

    # >>>><<< DONT KEEP THIS in this experiment
    #test_path = discriminator.preprocess_custom_folder(
    #    input_images_folder=f'{nnunet_folders_path}/nnUNet_raw/{nnunet_dataset}/imagesTr',# # imagesTr for experiment just changing labels as domain shift
    #    input_segs_folder=f'{nnunet_folders_path}/nnUNet_raw/{nnunet_dataset}/labelsTr_{args.test_annotator}', #labelsTr_2' for experiment just changing labels as domain shift
    #    folder_name='test'
    # )
    #test_path = "/tmp/tmpfunysoaf/test"
    #training_nnunet_dataset.transform_keys(lambda k: 'workaround_'+k)
    # DONT KEEP THIS in this experiment <<<<<<<<<<
    ...
    test_data_for_disc, test_data_for_infer = (
        out_of_domain_with_same_images.random_split(0.65)
    )

    train_data, val_data = training_nnunet_dataset.merge_and_split(
        test_data_for_disc, split_ratio=0.7, check_conflicting_cases = True #True always for original experiment
    )
    ...
    inference_env_perf_f1 = compute_weighted_metrics_merged_dataset(
        data_csv, test_data_for_infer, measures=["F1_score"]
    )

    train_env_perf_dice = compute_weighted_metrics_merged_dataset(
        data_csv, just_in_train_centers_dataset_unseen_for_transport, measures=["Dice"]
    )
    inference_env_perf_dice = compute_weighted_metrics_merged_dataset(
        data_csv, test_data_for_infer, measures=["Dice"]
    )
