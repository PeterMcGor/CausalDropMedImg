...
from attributions.core.mechanism_shift import CausalMechanismShift, CausalMechanismShiftMed
...
def main(args):
    ...
    nnunet_dataset = f'Dataset00{args.train_annotator}_MSSEG_FLAIR_Annotator{args.train_annotator}'
    ...
    # center 03 not in the source, want just label effect here
    just_in_train_centers_dataset_unseen_for_transport = test_nnunet_dataset.subset_by_pattern(exclude_center_pattern)
    ...
    analyzer = CausalMechanismShiftMed(
        ...
        causal_graph=dataset_graph.graph if isinstance(dataset_graph, nx.DiGraph) else dataset_graph.graph,
        shapley_config=ShapleyConfig(num_subset_samples=args.shapley_samples, n_jobs=args.n_jobs)
    )
    ...
    pivot_columns = ['initial_train_perf', 'initial_inference_perf', 'performance_difference', 'value']
    pivot_df = combined_df.pivot(index='mechanism', columns='metric', values=pivot_columns)
    ...
    combined_output = os.path.join(result_folder, f'{output_base}_combined_attributions.csv')
    pivot_output = os.path.join(result_folder, f'{output_base}_pivoted_attributions.csv')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run causal mechanism shift analysis with different annotators')

    parser.add_argument('--train_annotator', type=int, required=True, choices=range(1, 8),
                        help='Annotator number for training (1-7)')

    parser.add_argument('--test_annotator', type=int, required=True, choices=range(1, 8),
                        help='Annotator number for testing (1-7, different from train)')

    parser.add_argument('--nnunet_folders_path', type=str, default='/home/jovyan/nnunet_data/',
    ...
    parser.add_argument('--num_train_iterations_per_epoch', type=int, default=100,
                        help='validation freq.')

    parser.add_argument('--num_val_iterations_per_epoch', type=int, default=40,
                        help='validation freq.')
    ...
    parser.add_argument('--shapley_samples', type=int, default=500,
