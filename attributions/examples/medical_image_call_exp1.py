...
def main(args):
    ...
    training_config = TrainingConfig(
        ...
        log_path=None,
        save_path=None,
        exp_name=f"Exp1_Annotator{args.train_annotator}_vs_Annotator{args.test_annotator}"
    )
    ...
    #train_nnunet_path = "/tmp/tmpamds9ph3/train_data"
    ...
    #test_nnunet = "/tmp/tmpusimyxun/train_transport"
    ...
    nnunet_dataset_disc = (
        f"Dataset00{args.test_annotator}_MSSEG_FLAIR_Annotator{args.test_annotator}"
    )
    test_path = discriminator.preprocess_custom_folder(
        input_images_folder=f'{nnunet_folders_path}/nnUNet_raw/{nnunet_dataset}/imagesTr',#
        input_segs_folder=f'{nnunet_folders_path}/nnUNet_raw/{nnunet_dataset_disc}/labelsTr',
        folder_name='test'
    )
    #test_path = "/tmp/tmp6ir891yz/test"
   ...
   # )
    ...
    train_data, val_data = training_nnunet_dataset.merge_and_split(
        out_of_dataset, split_ratio=0.7, split_type=None,check_conflicting_cases = True #True always for original experiment
    )
    discriminator.set_datasets(
        train_data=train_data,
        val_data=val_data,
        test_dataset_train_domain=just_in_train_centers_dataset_unseen_for_transport,
        test_dataset_inference_domain=just_in_train_centers_dataset_unseen_for_transport,
    )
    ...
    inference_env_perf_f1 = compute_weighted_metrics_merged_dataset(
        data_csv_test_labels, just_in_train_centers_dataset_unseen_for_transport, measures=["F1_score"]
    )
    ...
    inference_env_perf_dice = compute_weighted_metrics_merged_dataset(
        data_csv_test_labels, just_in_train_centers_dataset_unseen_for_transport, measures=["Dice"]
    )
    ...
    output_base = f"Exp_1_annotator{args.train_annotator}_vs_annotator{args.test_annotator}"
