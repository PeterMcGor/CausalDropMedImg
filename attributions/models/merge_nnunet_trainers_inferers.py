...
class MergedNNUNetDataLoaderSpecs(DataLoaderSpecs):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            batch_keys=[
                MergerNNUNetDataset.BATCH_IMAGES_KEY,
                MergerNNUNetDataset.BATCH_SEG_KEY,
                MergerNNUNetDataset.BATCH_PROPERTIES_KEY,
            ],
            **kwargs,
        )
        ...
        assert isinstance(
            dataset_plans_path, str
        ), "dataset_plans_path must be a string"
        assert isinstance(
            dataset_train, MergerNNUNetDataset
        ), "dataset_train must be an instance of MergerNNUNetDataset"
        assert isinstance(
            dataset_val, MergerNNUNetDataset
        ), "dataset_val must be an instance of MergerNNUNetDataset"
    ...
    def validate_batch(self, batch: Dict[str, Any]) -> bool:
        ...
        if (
            batch[MergerNNUNetDataset.BATCH_IMAGES_KEY].shape[0]
            != batch[MergerNNUNetDataset.BATCH_SEG_KEY].shape[0]
        ):
            return False
...
class MergedNNUNetDatasetTrainer(BaseTrainerWithSpecs):
    ...
    def train(self):
        if self.dataloader_specs.unpack_data:  # from nnUNet code
            ...
            for folder in folders_unpack:
                unpack_dataset(
                    ...
                    num_processes=self.dataloader_specs.num_processes,
                    verify_npy=True,
                )
        ...
        results = super().train(
            self.dataloader_specs.train_loader, self.dataloader_specs.val_loader
        )
    ...
    def _process_batch(self, batch: Dict[str, Any]) -> tuple:
        ...
        # Move individual tensors to the device
        data = self._move_batch_to_device(batch["data"])
        target = self._move_batch_to_device(batch["target"])

        # Concatenate them after they're on the device
        inputs = torch.cat([data, target], dim=1)

        # Extract labels directly to the correct device
        labels = torch.tensor(
...
class MergedNNUNetDatasetTrainerImages(MergedNNUNetDatasetTrainer):
    ...
    def _process_batch(self, batch: Dict[str, Any]) -> tuple:
        ...
        # Move just the data tensor to the device
        inputs = self._move_batch_to_device(batch["data"])
...
class MergedNNUNetInference:
    ...
    def __init__(
        ...
        self.device = torch.device(
            config.device if isinstance(config.device, str) else config.device
        )
    ...
    def run_inference(self):
        ...
        with torch.no_grad():
            for batch_idx, preprocessed in enumerate(self.dataloader):
                ...
                # Store results de-batching the batch
                batch_result = {
                    case_id: {
                    ...
                    for i, case_id in enumerate(properties)
                }
    ...
    def _process_batch(self, batch: Dict[str, Any]) -> tuple:
        """Process batch from MergerNNUNetDataset format"""
        inputs = torch.cat([batch["data"], batch["target"]], dim=1).to(
            self.config.device
        )
        return inputs
